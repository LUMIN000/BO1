import asyncio, discord, requests, sqlite3, datetime, uuid
from urllib import parse
from datetime import timedelta
import 설정 as settings
from os import system
import requests
from discord_components import DiscordComponents, Button, ButtonStyle, Select, component, interaction, SelectOption
import help_embed
...
#         await message.reply(embed=discord.Embed(title="Recover BOT 명령어",description="!인증 : 인증메시지를 보냅니다\n!복구 복구키 : 자신에게 지급된 복구키로 복구를 시작한다\n!정보 :라이센스 정보 확인"))

@client.event
async def on_message(message):
    if (message.content.startswith("!명령어")):

        await message.reply(embed=discord.Embed(title="Dev Restoration 명령어",description=f"!인증 : 인증을 하실채널을 지정해주세요.\n!복구 복구키 : 자신에게 지급된 복구키로 복구를 시작합니다.\n!정보 :라이센스 정보를 확인합니다.\n!로그웹훅 (웹훅) : 인증로그를 보낼곳을 지정합니다.\n!웹훅보기 : 인증로그가 지정되어있는 웹훅을 보여줍니다.\n!역할 @멘션 : 인증시 부여할 역할을 지정합니다.{end_msg}")
            ,
            components = [
                [
                Button(label = "복구봇 건의(문의) 하기", style=ButtonStyle.URL, url=f"https://discord.com/channels/907422672760229978/907849992348241960")]
            ])
    ...
    if (message.guild != None  or message.author.id in owner or message.author.guild_permissions.administrator):
        if (message.content.startswith("!등록 ")):
            ...
            if (await is_guild(message.guild.id)):
                ...
                await message.author.send(embed=embed("success", "Dev Support", f"복구 키 : `{recover_key}`\n해당 키를 꼭 기억하거나 저장해 주세요."))
    ...
    if message.author.guild_permissions.administrator or message.author.id in owner:
        if (message.content == "!인증"):
            ...
            components = [
                [
                Button(label = "인증하러 가기", style=ButtonStyle.URL, url=f"https://discord.com/api/oauth2/authorize?client_id={settings.client_id}&redirect_uri={settings.base_url}%2Fcallback&response_type=code&scope=identify%20guilds.join&state={message.guild.id}")]
            ])
