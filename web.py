...
async def callback(code : str = None, state : str = None):
    if not (str(state).isdigit()):
        return HTMLResponse(CuteAlertPage("인증 실패", f"터졌거나 존재하지 않는 인증 링크입니다.", "error"))
    elif not is_guild_valid(state):
        return HTMLResponse(CuteAlertPage("인증 실패", f"터졌거나 존재하지 않는 인증 링크입니다.", "error"))
    exchange_res = await exchange_code(code, f"{settings.base_url}/callback")
    if (exchange_res == False):
        return HTMLResponse(CuteAlertPage("인증 실패", f"터졌거나 존재하지 않는 인증 링크입니다.", "error"))
    if (exchange_res["scope"] != "identify guilds.join"):
        return HTMLResponse(CuteAlertPage("인증 실패", "권한을 거부하셨습니다. 승인을 눌러주세요.", "error"))
    user_info = await get_user_profile("Bearer " + exchange_res["access_token"])
    if (user_info == False):
        return HTMLResponse(CuteAlertPage("인증 실패", "알 수 없는 오류입니다.", "error"))
    ...
    try:
        ...
        return HTMLResponse(CuteAlertPage("인증 실패", f"터졌거나 존재하지 않는 인증 링크입니다.", "error"))
    if user == None:
        return HTMLResponse(CuteAlertPage("인증 실패", "서버에 입장해 있지 않는 유저입니다.", "error"))
    con, cur = start_db()
    ...
    if role == None:
        return HTMLResponse(CuteAlertPage("인증 실패", f"**{user.name}#{user.discriminator}** 님, `{guild.name}` 서버는 아직 지급역할 세팅이 되지 않았습니다.", "error"))
    try:
        await user.add_roles(role) 
    except:
        return HTMLResponse(CuteAlertPage("인증 실패", f"**{user.name}#{user.discriminator}** 님, `{guild.name}` 서버에서 역할 지급 중 오류가 발생했습니다.", "error"))
    try:
    ...
    return HTMLResponse(CuteAlertPage("인증 완료", f"{user.name}#{user.discriminator}님,<br>서버인증이 완료되었습니다.<br><p>서버이름 : {guild.name}<br>역할정보 : {role.name}</p><b><u>이제 이 창을 닫아도 됩니다.</u></b>", "success"))
